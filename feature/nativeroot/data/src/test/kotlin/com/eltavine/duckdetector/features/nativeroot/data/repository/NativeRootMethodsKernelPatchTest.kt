...
class NativeRootMethodsKernelPatchTest {

    private fun latencyRow(snapshot: NativeRootNativeSnapshot): NativeRootMethodResult = buildMethods(
        snapshot = snapshot,
        findings = emptyList(),
        shellTmpDetail = "",
        rootProcessDetail = "",
    ...
    ).single { it.label == "__NR_supercall probe" }
    ...
    fun `a measured run without the pre-fix delay is clean`() {
        val row = latencyRow(NativeRootNativeSnapshot(available = true, kernelPatchSideChannelAvailable = true))
