...
fun TeeReport.toDetectorResult(): DetectorResult = DetectorResult(
    id = TeeDetectorId,
    status = toDetectorStatus(),
    // Expansion only changes the card, never the report.
    report = TeeCardModelMapper().map(this, isExpanded = false).toDetectorReport(),
)
