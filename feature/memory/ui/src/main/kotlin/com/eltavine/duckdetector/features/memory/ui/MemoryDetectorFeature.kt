...
package com.eltavine.duckdetector.features.memory.ui
...
class MemoryDetectorFeature(
    private val createScanner: (Context) -> DetectorScanner<MemoryReport>,
) : DetectorFeature {
    ...
    override fun rememberSession(): DetectorSession {
        ...
        val viewModel: MemoryViewModel = viewModel(
            factory = remember(context) { MemoryViewModel.factory { createScanner(context.applicationContext) } },
        )
        return remember(viewModel) { MemoryDetectorSession(viewModel) }
    }
}

private class MemoryDetectorSession(
    private val viewModel: MemoryViewModel,
) : DetectorSession {
    override val id: DetectorId = MemoryDetectorId
    ...
    override fun Card() {
        ...
        MemoryDetectorCard(model = state.cardModel)
    }
}
