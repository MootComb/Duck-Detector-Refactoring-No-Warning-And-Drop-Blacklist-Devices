...
package com.eltavine.duckdetector.features.zygisk.ui
...
object ZygiskDetectorFeature : DetectorFeature {
    override val id: DetectorId = ZygiskDetectorId

    @Composable
    override fun rememberSession(): DetectorSession {
        val context = LocalContext.current
        val viewModel: ZygiskViewModel = viewModel(factory = remember(context) { ZygiskViewModel.factory(context) })
        return remember(viewModel) { ZygiskDetectorSession(viewModel) }
    }
}

private class ZygiskDetectorSession(
    private val viewModel: ZygiskViewModel,
) : DetectorSession {
    override val id: DetectorId = ZygiskDetectorId
    ...
    override fun Card() {
        ...
        ZygiskDetectorCard(model = state.cardModel)
    }
}
