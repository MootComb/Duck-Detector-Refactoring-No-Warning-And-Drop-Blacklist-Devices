...
object BootloaderDetectorFeature : DetectorFeature {
    override val id: DetectorId = BootloaderDetectorId

    @Composable
    override fun rememberSession(): DetectorSession {
        val context = LocalContext.current
        val viewModel: BootloaderViewModel = viewModel(factory = remember(context) { BootloaderViewModel.factory(context) })
        return remember(viewModel) { BootloaderDetectorSession(viewModel) }
    }
}

private class BootloaderDetectorSession(
    private val viewModel: BootloaderViewModel,
) : DetectorSession {
    override val id: DetectorId = BootloaderDetectorId
    ...
    override fun Card() {
        ...
        BootloaderDetectorCard(model = state.cardModel)
    }
}
